...
export function $(_name: string, _attr?: { [key: string]: string }) {
    ...
    function wrap(..._args: Array<typeof wrap | typeof activate | string>): typeof activate {

        function activate(selector?: { [key: string]: (string | typeof activate) }, hasEndTag: boolean = true): string {
            ...
            for (let i = 0; i < args.length; i++) {
                ...
                if (typeof arg == 'function') {
                    if (arg.hasOwnProperty(wrapper)) {
                        part = part + ((arg as typeof wrap)() as typeof activate)(selector, false);
                    }
                    else if (arg.hasOwnProperty(activator)) {
                        if (isPreamble) {
                            hasEndTag = false;
                        }
                        part = part + (arg as typeof activate)(selector, true);
                    }
                }
